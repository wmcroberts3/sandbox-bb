		...
		implements WebSocket
{
			...
			throws WebSocketException
	{
		try {
			...
			output = new PrintStream(socket.openOutputStream());
			...
			Vector handshakeLines = new Vector();
			...
			while (!handshakeComplete) {
				...
				if (!header) {
					...
					if (line.trim().equals("")) {
						...
						handshakeLines.addElement(line.trim());
					}
			...
			handshake.verifyServerStatusLine((String)handshakeLines.elementAt(0));
			handshake.verifyServerResponse(serverResponse);
			
			// handshakeLines.remove(0);
			handshakeLines.removeElementAt(0);
			
			// HashMap<String, String> headers = new HashMap<String, String>();
			Hashtable headers = new Hashtable();
			
			// for (String line : handshakeLines) {
			int countLines = handshakeLines.size();
			for (int i = 0; i < countLines; i++)
			{
				String line = (String)handshakeLines.elementAt(i);
				
				// String[] keyValue = line.split(": ", 2);
				String[] keyValue = Utilities.split(line, ": ");
			...
			throws WebSocketException
	{
		...
		if (scheme != null && scheme.equals("ws")) {
			...
			try {
				...
				socket = (SocketConnection) Connector.open(scheme + host + portString, Connector.READ_WRITE);
			}
